...
AstroPlanAI is a multi-agent astrophotography planning system using local VLLM models via OpenAI-compatible APIs. It orchestrates specialized agents (weather, ephemeris, target selection, scheduler) to create optimal imaging session plans.
...
cp .env.template .env
# Edit .env with your VLLM endpoint and model
```

### Running Examples
```bash
# Full multi-agent planning demo (requires VLLM running)
...
**Note**: The project expects VLLM to be running externally on the host machine.
...
**VLLM Connection**: Containers connect to VLLM running on the host machine via `host.docker.internal` (configured automatically on Linux via `extra_hosts`).
...
**Key Implementation Detail**: The coordinator runs weather, ephemeris, and target selection agents in parallel using `asyncio.gather()` (coordinator.py:112-116), then feeds consolidated results to the scheduler agent sequentially.
...
Each specialized agent (weather.py, ephemeris.py, target_selection.py, scheduler.py) defines:
...
- `ephemeris_calculator.py`: AstroPy-based astronomical calculations
- `weather_api.py`: Open-Meteo API integration for forecasts
- `target_database.py`: Curated DSO database with seasonal filtering
...
- `VLLMConfig`: VLLM endpoint, API key, model name
...
**Architecture**: This project runs as a containerized application that connects to an **external VLLM server** running on the host machine.

**Requirements**:
- VLLM server must be running on host machine (see VLLM_SETUP.md)
- VLLM must expose OpenAI-compatible API endpoint (default: http://localhost:8000)
- Configure connection in `.env` file

**Container Connectivity**:
- The Docker container uses `host.docker.internal` to reach the host machine
- Default endpoint: `http://host.docker.internal:8000/v1`
- The entrypoint script validates VLLM connectivity on startup

**Configuration**: Set VLLM_BASE_URL, VLLM_API_KEY, VLLM_MODEL in `.env`

The OpenAI Python SDK (openai>=1.12.0) is used to communicate with VLLM via the `/v1/chat/completions` endpoint.
...
### Async/Await Pattern
All agent interactions use async/await. The coordinator's `plan_session()` method is the main entry point and must be called with `asyncio.run()`.
...
Required:
- `VLLM_MODEL`: Model name loaded in VLLM (e.g., "meta-llama/Llama-3.1-8B-Instruct")

Optional:
- `VLLM_BASE_URL`: Default "http://localhost:8000/v1"
- `VLLM_API_KEY`: Default "not_required_for_local"
